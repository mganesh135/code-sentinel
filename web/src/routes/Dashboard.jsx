...
function SectionHeader({ eyebrow, title, subtitle }) {
  return (
    <div style={{ marginBottom: 18 }}>
      {eyebrow && (
        <div style={{ fontSize: 11.5, color: 'var(--accent-hover)', fontWeight: 510, letterSpacing: 0.08, textTransform: 'uppercase', marginBottom: 8 }}>
          {eyebrow}
        </div>
      )}
      <h2>{title}</h2>
  ...
  return (
    <AppShell>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 36 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 24, flexWrap: 'wrap' }}>
            <div style={{ flex: '1 1 380px' }}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 22, flexWrap: 'wrap' }}>
                ...
                <button className="btn btn-ghost btn-lg" onClick={() => navigate('/subscriptions')}>
                  View plans
                </button>
              </div>
            </div>
            ...
            <div style={{ flex: '0 0 420px', minWidth: 320 }}>
              <div style={{ background: '#15140f', borderRadius: 12, overflow: 'hidden', border: '1px solid #2a2722', boxShadow: '0 30px 60px -20px rgba(20,18,14,0.25)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '10px 14px', borderBottom: '1px solid #2a2722' }}>
                  <span style={{ width: 10, height: 10, borderRadius: 999, background: '#3a352e' }} />
                  <span style={{ width: 10, height: 10, borderRadius: 999, background: '#3a352e' }} />
                  <span style={{ width: 10, height: 10, borderRadius: 999, background: '#3a352e' }} />
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: '#6f6a5e', marginLeft: 8 }}>~/vector-app</span>
                </div>
        ...
        {/* ── Downloads ── */}
        <section>
          <SectionHeader eyebrow="Downloads" title="Get the CLI" subtitle="Pick a native installer or use your favorite package manager." />
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14, marginBottom: 18 }}>
            {platforms.map((p) => (
              <div key={p.id} className="card" style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{ width: 38, height: 38, borderRadius: 9, background: 'var(--surface-2)', border: '1px solid var(--border)', display: 'grid', placeItems: 'center', color: 'var(--text)' }}>
                    {p.icon}
                  </div>
                  <div>
                ...
                <div className="mono" style={{ fontSize: 11.5, color: 'var(--text-3)', padding: '7px 10px', background: 'var(--surface)', borderRadius: 6, border: '1px solid var(--border)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                  {p.cmd}
                </div>
          ...
          <div className="card" style={{ padding: 18 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 12 }}>
              {installers.map((i) => (
                <button key={i.id} onClick={() => setTab(i.id)}
                  style={{
                    ...
                    fontWeight: tab === i.id ? 510 : 450, cursor: 'pointer',
                  }}
                >
                  {i.icon} {i.label}
                </button>
              ...
              <div style={{ flex: 1 }} />
              <button className="btn btn-quiet btn-sm"><Icon.Copy size={12} /> Copy</button>
            </div>
        ...
        <section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 14 }}>
            {[
              ...
              <a key={i} className="card" style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 8, cursor: 'pointer', transition: 'border-color 0.12s', textDecoration: 'none' }}
